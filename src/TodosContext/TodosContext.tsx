...
export const TodosProvider: React.FC<Props> = ({ children }) => {
    ...
    todoService
      ...
      .then(() => {
        ...
        setProcessingIds(prev =>
          prev.filter(currTodoId => currTodoId !== todoId),
        );
      })
      .catch(() => {
        setIsSubmiting(false);
      ...
      if (startValue !== editedTodoTitle) {
        ...
        todoService
          ...
          .then(() => {
            ...
            setProcessingIds(prev =>
              prev.filter(currentTodo => currentTodo !== todoId),
            );
          })
          .catch(() => {
            ...
            setEditedTodoId(todoId);
            setProcessingIds(prev =>
              prev.filter(currTodo => currTodo !== todoId),
            );
          });
      } else {
        setEditedTodoId(null);
      ...
      if (startValue !== editedTodoTitle) {
        ...
        todoService
          ...
          .then(() => {
            ...
            setProcessingIds(prev =>
              prev.filter(currentTodoId => currentTodoId !== todoId),
            );
          })
          .catch(() => {
            setErrorMessage(ErrorMessage.UnableToUpdateATodo);
  ...
  const handleCompleteChange = (todoId: number, checked: boolean) => {
    setProcessingIds(curr => [...curr, todoId]);
    todoService
      .editTodo(todoId, { completed: checked })
      .then(() => {
