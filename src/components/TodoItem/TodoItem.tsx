...
export const TodoItem: React.FC<Props> = React.memo(({
  ...
  const CurrentTodoUpdating = todoIdsInUpdating.includes(todo.id);
  ...
  const handleTodoDelete = () => {
    ...
    deleteTodo(todo.id)
      ...
      .catch(() => {
        setError('Error while deleting a todo');
      })
      .finally(() => {
        setIsLoading(false);
      });
  ...
  const handleTodoEdit = (data: TodoUpdate) => {
    ...
    updateTodo(todo.id, data)
      ...
      .catch(() => {
        setError('Error while updating a todo');
      })
      .finally(() => {
        setIsLoading(false);
      });
  ...
  }, [inputTitleValue, todo.title, handleTodoEdit, handleTodoDelete]);
    ...
    <div className={classNames('todo', { completed: todo.completed })}>
      ...
      <div
        className={classNames('modal', 'overlay', {
          'is-active':
            isPermanentlyLoading || isLoading || CurrentTodoUpdating,
        })}
      >
