...
export const App: React.FC = () => {
  ...
  const handleErrorDelete = () => setError('');
  ...
  const deleteTodo = async (currentId: number) => {
    try {
      setProcessingTodoIds(prevIds => [...prevIds, currentId]);
  ...
  const updateTodo = async (updatedTodo: Todo) => {
    try {
      setProcessingTodoIds(prevIds => [...prevIds, updatedTodo.id]);
                ...
                return (
                  <div className={classNames('todo', { completed })} key={id}>
                    ...
                    {isTodoEditing && todoEditing?.id === id ? (
                      <form onSubmit={(
                        event,
                      ) => changeTodoTitle(event, newTitle)}
                      >
                        <input
                          type="text"
          ...
          {todos.length > 0 && (
            <footer className="todoapp__footer">
              ...
              <nav className="filter">
                ...
                <a
                  href="#/active"
                  className={classNames('filter__link', 'filter__link__active',
              ...
              <button
                ...
                className={classNames('todoapp__clear-completed',
                  'clear-completed', {
                    'clear-completed__hide': completedTodos.length
                  === 0,
                  })}
